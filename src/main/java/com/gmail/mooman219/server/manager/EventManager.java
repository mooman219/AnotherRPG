...
import com.gmail.mooman219.server.event.EventExecutor;
import com.gmail.mooman219.server.event.HandlerList;
import com.gmail.mooman219.server.event.Listener;
import com.gmail.mooman219.server.event.ListenerRegistration;
...
public class EventManager{
    ...
    public static void registerEvent(Class<? extends Event> event, Order priority, EventExecutor executor) {
        getEventListeners(event).register(new ListenerRegistration(executor, priority));
    }
        ...
        if (listeners != null) {
            if(event instanceof CancellableEvent){
                for (ListenerRegistration listener : listeners) {
                    try {
                        if(!(((CancellableEvent) event).isCancelled()) || listener.getOrderSlot().ignoreCancelled){
